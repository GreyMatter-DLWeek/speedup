export function initFeature7Legacy(ctx) {
  ...
  function populateSourceSelectors() {
    const rows = Array.isArray(runtime.state.practiceUploads) ? runtime.state.practiceUploads : [];
    const selectors = ["legacyQuizSourceSelect", "legacyFlashcardSourceSelect"];
  ...
  function renderUploads() {
    const wrap = document.getElementById("legacyUploadedPapersList");
    if (!wrap) return;
    const rows = Array.isArray(runtime.state.practiceUploads) ? runtime.state.practiceUploads : [];
    if (!rows.length) {
    ...
    const activeQuizIndex = Number(document.getElementById("legacyQuizSourceSelect")?.value ?? 0);
    wrap.innerHTML = rows
      .slice(0, 20)
      .map((item, idx) => {
  ...
  function renderInsights(latestAnalysis) {
    const insightEl = document.getElementById("legacyPracticeInsights");
    if (!insightEl) return;
  ...
  function renderAnalysis(analysis, provider) {
    const out = document.getElementById("legacyPracticeAnalysis");
    if (!out) return;
    if (!analysis) {
  ...
  function renderQuiz(out) {
    const wrap = document.getElementById("legacyQuizOutput");
    if (!wrap) return;
  ...
  function renderFlashcards(out) {
    const wrap = document.getElementById("legacyFlashcardOutput");
    if (!wrap) return;
  ...
  async function onAnalyzePaper() {
    const statusEl = document.getElementById("legacyPracticeStatus");
    const fileInput = document.getElementById("legacyPracticeFileInput");
    const topicInput = document.getElementById("legacyPracticeTopicInput");
    const pastedInput = document.getElementById("legacyPracticePastedText");
    ...
    try {
      ...
      document.getElementById("legacyQuizSourceSelect").value = "0";
      document.getElementById("legacyFlashcardSourceSelect").value = "0";
      renderUploads();
    } catch (error) {
  ...
  async function onGenerateQuiz() {
    const status = document.getElementById("legacyQuizStatus");
    const difficulty = String(document.getElementById("legacyQuizDifficulty")?.value || "medium");
    const numQuestions = Math.max(1, Math.min(20, Number(document.getElementById("legacyQuizCount")?.value || 5)));
    const questionType = String(document.getElementById("legacyQuizType")?.value || "mcq");
    const sourceText = getSourceTextFor("legacyQuizSourceSelect");
  ...
  async function onGenerateFlashcards() {
    const status = document.getElementById("legacyFlashcardStatus");
    const count = Math.max(1, Math.min(30, Number(document.getElementById("legacyFlashcardCount")?.value || 8)));
    const sourceText = getSourceTextFor("legacyFlashcardSourceSelect");
  ...
  function initPracticeFeature() {
    ...
    document.getElementById("legacyPracticeAnalyzeBtn")?.addEventListener("click", onAnalyzePaper);
    document.getElementById("legacyGenerateQuizBtn")?.addEventListener("click", onGenerateQuiz);
    document.getElementById("legacyGenerateFlashcardsBtn")?.addEventListener("click", onGenerateFlashcards);
    document.getElementById("legacyQuizSourceSelect")?.addEventListener("change", renderUploads);
    document.getElementById("legacyFlashcardSourceSelect")?.addEventListener("change", renderUploads);
